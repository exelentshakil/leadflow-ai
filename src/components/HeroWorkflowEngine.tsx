...
export function HeroWorkflowEngine() {
  ...
  const [stepStatus, setStepStatus] = useState({
    firewall: 'PASSED (0ms) • Zero PII Exposure',
    inference: '78ms • Claude 3.5 & GPT-4o-mini',
    schema: 'ENFORCED • CRM Sync & Slack Block Kit',
  });
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setResult(data);
        setStepStatus({
          firewall: data.firewallStatus?.passed ? 'PASSED (0ms) • Zero PII Leak' : 'SECURITY INTERCEPT (0ms)',
          inference: `${data.latencyMs || 78}ms • ${data.provider} (${data.model || 'gpt-4o-mini'})`,
          schema: 'ENFORCED (1ms) • CRM + Slack Block Kit',
        });
      } else {
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
            ...
            <Button
              ...
              disabled={loading}
              className="h-9 px-4 text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white shadow-xs shrink-0 cursor-pointer"
            >
              {loading ? (
                <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-3 text-xs font-mono">
            ...
            <div className="flex items-center gap-2 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] p-2.5 shadow-2xs">
              ...
              <div className="min-w-0">
                <div className="text-[10px] text-[var(--color-text-muted)] font-sans uppercase">Stage 1 • LLM Security &amp; PII Scrub</div>
            ...
            <div className="flex items-center gap-2 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] p-2.5 shadow-2xs">
              ...
              <div className="min-w-0">
                <div className="text-[10px] text-[var(--color-text-muted)] font-sans uppercase">Stage 2 • Claude &amp; GPT Lead Scoring</div>
            ...
            <div className="flex items-center gap-2 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] p-2.5 shadow-2xs">
              ...
              <div className="min-w-0">
                <div className="text-[10px] text-[var(--color-text-muted)] font-sans uppercase">Stage 3 • CRM Sync &amp; Slack Block Kit</div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-[var(--color-text-primary)] font-mono">
                  Verified Pipeline Output (Structured JSON Payload)
                </span>
              </div>
              <Button
                variant="ghost"
