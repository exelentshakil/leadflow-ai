...
// 4 Distinct Wavy Sparkline Datasets for Lead Automation Telemetry
const leadIngestionTrend = [
  { t: '08:00', v: 1240 },
  { t: '10:00', v: 2480 },
  { t: '12:00', v: 3820 },
  { t: '14:00', v: 4950 },
...
// High-Density Multi-Step Workflow Pipeline Telemetry Stream
const telemetryStream = [
  { time: '09:00', ops: 3820, latency: 14.8, stage: 'Webhook Intake & Form Validation' },
  { time: '10:00', ops: 4790, latency: 14.1, stage: 'Inline LLM Security & PII Redaction' },
  { time: '11:00', ops: 4210, latency: 13.9, stage: 'Claude / GPT Lead Scoring (0-100)' },
  { time: '12:00', ops: 5680, latency: 14.4, stage: 'Inquiry Summarization & Intent Extraction' },
  { time: '13:00', ops: 5120, latency: 13.8, stage: 'Personalized Draft Reply Synthesis' },
  { time: '14:00', ops: 6450, latency: 13.2, stage: 'CRM Sync (HubSpot / Sheets / Airtable)' },
  { time: '15:00', ops: 5890, latency: 13.5, stage: 'Slack Block Kit Notification Dispatch' },
  { time: '16:00', ops: 6820, latency: 12.8, stage: '1-Click Human Approval Gateway' },
  { time: '17:00', ops: 6180, latency: 13.4, stage: 'Email Outbound Dispatch' },
  { time: '18:00', ops: 6450, latency: 13.2, stage: 'Audit Log & Failure Alert Check' },
];
...
export function MetricsGrid() {
  ...
  const renderCardChart = (idx: number) => {
    ...
    if (idx === 0) {
      // Card 0: Lead Ingestion Velocity (Stripe Blurple wavy gradient area)
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={leadIngestionTrend} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="rounded-[4px] bg-[var(--color-surface)] border border-[var(--color-border)] px-2.5 py-1 text-xs font-medium shadow-xs text-[var(--color-text-primary)]">
                      <span className="font-bold text-[#533AFD] dark:text-[#7A68FF]">{payload[0].value?.toLocaleString()}</span> leads/hr
                    </div>
                  );
                }
                return null;
    ...
    if (idx === 1) {
      // Card 1: AI Qualification Accuracy (Emerald wavy gradient area)
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={aiQualificationAccuracy} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
            ...
            <Tooltip
              content={({ active, payload }) => {
                if (active && payload && payload.length) {
                  return (
                    <div className="rounded-[4px] bg-[var(--color-surface)] border border-[var(--color-border)] px-2.5 py-1 text-xs font-medium shadow-xs text-[var(--color-text-primary)]">
                      <span className="font-bold text-[#057A55] dark:text-emerald-400">{payload[0].value}%</span> Precision
                    </div>
                  );
                }
                return null;
    ...
    if (idx === 2) {
      // Card 2: CRM Sync Delivery SLA (Amber wavy gradient area)
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={crmSyncDeliverySla} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
    ...
    // Card 3: Slack Approval Turnaround Velocity (Teal wavy gradient area)
    return (
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={approvalTurnaroundTrend} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
          <defs>
        ...
        {siteConfig.metrics.map((metric, idx) => {
          ...
          return (
            ...
            >
              ...
              {/* Card Body: Primary Bold Metric & Wavy Sparkline */}
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm sm:text-[15px] font-bold tracking-tight text-[var(--color-text-primary)]">
                n8n &amp; Make Workflow Pipeline • Sub-50ms Handoff Telemetry
              </span>
            ...
            <p className="text-xs sm:text-sm text-[var(--color-text-secondary)] mt-1">
              Real-time lead ingestion throughput across Webhook Intake → LLM Scoring → CRM Sync → Slack Approval
            </p>
          </div>
